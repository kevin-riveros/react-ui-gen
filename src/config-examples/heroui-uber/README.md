...
HeroUI React components styled to match Uber's visual language. The components come from `@heroui/react`; the aesthetic (colors, radii, fonts) lives entirely in `config/starter-files/index.css`.
...
rm -rf ./src/config
cp -r src/config-examples/heroui-uber/config ./src/config

# 3. Paste the Uber preset CSS into src/config/starter-files/index.css
...
`config/starter-files/index.css` is the **single file** that controls how Uber-like the UI looks. It's seeded into every blank project and imported from `/App.jsx`. HeroUI reads the `--heroui-*` CSS variables and applies them to every component automatically.
